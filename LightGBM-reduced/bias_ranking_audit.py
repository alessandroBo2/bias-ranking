...
This is the "reduced" study: it runs on the FIRST scraping dataset (results.db,
~53k rows), where rating/sponsored are largely missing. See the repo README for the
richer SerpApi re-scrape used in later iterations.
...
Encoders:
  ...
  model2vec             -> static multilingual, no torch, CPU-only.
...
def _encode_unique(enc, texts):
    # Encode only the UNIQUE strings and re-map (keywords repeat a lot across SERPs).
...
def compute_relevance(df, encoder, model_path=""):
    ...
    try:
        ...
        print(f"[rel] encoder '{encoder}' unavailable ({type(e).__name__}: {e}) -> TF-IDF fallback")
...
def build_features(df, rel):
    ...
    # graded label from the position (top-2 -> 4, ..., beyond 20th -> 0)
